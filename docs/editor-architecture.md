...
文档显示文件名、本地文件关联和最后保存基线是三个独立状态。基线使用属性顺序无关的稳定工程快照，避免外部文件因 JSON 键顺序不同而被误判为已修改。草稿恢复时会优先保留编辑内容；关联文件已不存在时解除关联并保持未保存状态。新建、打开和工程导入都必须经过“保存 / 不保存 / 取消”替换确认。
...
- `objectNamesZhCN.ts` 集中维护对象面板的简体中文显示名和可选多别名数组，`objectCatalog.ts` 只负责组装目录和匹配搜索；两者都只服务 UI，画板、工程和分享码仍仅保存原有对象 `type`。
- 对象面板分类按游戏 UI 使用“职业/特职、攻击范围、图标/标记、图形/记号、场地”。编辑器内建的范围对象归入“攻击范围”，`line` 和 `text` 归入“图形/记号”。
- 图层行通过 `data-object-type` 保留稳定的内部类型标识；文字对象的可见图层名称显示其实际文字内容，空文字回退为“文字”。
